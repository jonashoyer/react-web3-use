...
export const useERC20 = ({ tokenAddress, tokenContract, contractAddress, onApproval, onRevocation, approvalAmount, ...options }: UseERC20Options) => {

  // eslint-disable-next-line react-hooks/rules-of-hooks
  const contract = tokenContract ?? useContract({ address: tokenAddress, contractInterface: contractInterfaceERC20 });

  
  const { balance, loading: loadingBalance, refetch: refetchBalance } = useERC20Balance({ tokenContract: contract, ...options });
  const { allowance, loading: loadingAllowance, refetch: refetchAllowance } = useERC20Allowance({ tokenContract: contract, contractAddress, ...options });
